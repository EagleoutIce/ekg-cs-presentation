...
\def\PinguBCode{\pingu[body=btdl@color@primary,eyes shiny,feet sit, wings grab, cup=black]}
...
\savebox\pinguBGray{\tikz{\pingu[body=btdl@color@primary,eyes shiny,feet sit, wings grab, cup=black,:mix-all=\MixinOnGray]}}
...
\begin{align*}
   ...
   \onslide<3->{\q{$\Psi$}} &\only<3|handout:0>{= a \cdot \q{00} + b \cdot \q{11}}\only<4-|handout:1>{= a \cdot \q{10} + b \cdot \q{01}}
\end{align*}
      };
   }
   \onslide<4->{
...
\begin{frame}[c]
\begin{lrbox}\Waves
   \begin{tikzpicture}[z={(10:10mm)},x={(-60:6.5mm)},y=6.5mm,help/.style={gray!30,line cap=round},Arr/.style={{Kite[scale=.6]}-{Kite[scale=.6]}}]
...
\end{lrbox}
\begin{tikzpicture}[@O]
   \node[scale=.9] at (current page.center) {\usebox\Waves};
\end{tikzpicture}
\end{frame}
}
...
\begin{tikzpicture}[@O]
   \coordinate(c) at (current page.center);
   \def\Shift{0cm}
   \only<3->{\def\Shift{-4cm}}
   \onslide<2->{
      \draw[very thick,Kite-Kite]      ([xshift=\Shift-2cm]c) -- ++(2,0);
      \draw[gray] ([xshift=\Shift-1cm,yshift=-1cm]c) -- ++(0,2);

      \draw[gray]      ([xshift=\Shift+1cm]c) -- ++(2,0);
      \draw[very thick, Kite-Kite] ([xshift=\Shift+2cm,yshift=-1cm]c) -- ++(0,2);
   }

   \onslide<3->{
      \draw[lightgray] (current page.south) -- (current page.north);
   }

   \onslide<4->{%
   \draw[gray] ([xshift=5cm]c)++(45:1cm) -- ++(45:-2cm);
   \draw[very thick, Kite-Kite] ([xshift=5cm]c)++(-45:1cm) -- ++(-45:-2cm);

   \draw[very thick, Kite-Kite] ([xshift=2cm]c)++(45:1cm) -- ++(45:-2cm);
   \draw[gray] ([xshift=2cm]c)++(-45:1cm) -- ++(-45:-2cm);
   }
   \onslide<4->{
      \node[below=5mm] at ([xshift=-5cm,yshift=-1cm]c) {\usebeamerfont{subtitle}0};
      \node[below=5mm] at ([xshift=-2cm,yshift=-1cm]c) {\usebeamerfont{subtitle}1};
      \node[below=5mm] at ([xshift=2cm,yshift=-1cm]c) {\usebeamerfont{subtitle}0};
      \node[below=5mm] at ([xshift=5cm,yshift=-1cm]c) {\usebeamerfont{subtitle}1};
   }
\end{tikzpicture}
\end{frame}
