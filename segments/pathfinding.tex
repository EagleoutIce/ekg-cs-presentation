...
current/.style={fill=btdl@color@background,draw=black,text=black},%
queue/.style={fill=gray},
visited/.style={fill=btdl@color@background,draw=gray,text=gray}
]
...
\begin{onlyenv}<7-|handout:2>
   ...
   \On<10-12>{b}{current}\On<13->{b}{visited}
   \On<13-16>{e}{current}\On<17->{e}{visited}
   \On<17-18>{d}{current}\On<19->{d}{visited}
   \On<19>{a}{current}\On<20->{a}{visited}
   \On<20>{c}{current}\On<21->{c}{visited}
   \On<21-22>{h}{current}\On<23->{h}{visited}
   \On<23-24>{i}{current}\On<25->{i}{visited}
   \On<25->{j}{current}

   % queue up there
   \scope[every node/.append style={outer xsep=4pt}]
   \only<9->{
      \node[dot,b,xshift=-3cm,below=3mm] (q1) at (current page.north) {b};
   }
   \only<11->{
