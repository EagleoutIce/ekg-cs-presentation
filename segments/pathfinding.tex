...
\end{lrbox}
\begin{tikzpicture}[@O]
   \node[scale=.9] at ([yshift=-5mm]current page.center) {\usebox\FinalDijkstra};
\only<5->{
...
\only<35->{
   \node[above] at(current page.south) {\usebeamerfont{subtitle}Dijkstra-Algorithmus};
}
\end{tikzpicture}
\end{frame}
...
\end{frame}
\begin{frame}
   TODO: Pseudocode und analyse zum nachschlagen
\end{frame}
\begin{frame}
   TODO: Recap
\end{frame}
