...
\begin{tikzpicture}[@O]
   ...
   \onslide<9->{
      \draw[-Kite] (ta.east) to[edge node={node[above,sloped,pos=.2] {\(g^a \bmod p = 2\)}}] ([yshift=-1.25cm]ta-|tb.west) coordinate (ca);
      \draw[-Kite] (tb.west) to[edge node={node[above,sloped,pos=.2] {\(g^b \bmod p = 5\)}}] ([yshift=-1.25cm]tb-|ta.east) coordinate (cb);
   }
   \onslide<10->{
